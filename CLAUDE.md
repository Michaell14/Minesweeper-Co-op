...
| Mine/flag sprites, per palette | art in `components/ds/spriteArt.ts` (PURE — importable by `lib/`), drawn by `sprites.tsx` (the `<symbol>` defs and the `<Sprite>` that points at them); grids are 16x16 character rows like `icons.tsx` (shared renderer in `pixelArt.ts`). Pinnable via `settings.spriteSet` (picker: `components/SpriteSetCards.tsx`, wiring: `components/SpriteDefsHost.tsx`). Every set is on `/ds` |
