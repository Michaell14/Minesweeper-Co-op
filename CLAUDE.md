...
- **A socket handler is a ROW in `server/routes/index.js`, not a `socket.on` call.** The row declares its payload check (`validate`, a pure predicate from `server/validation.js`) and who may send it (`guard`); `routes/register.js` applies rate limit → validate → guard → handler for every route, so that order is structural rather than remembered. Add new payload rules to `validation.js` rather than inline. Pick `GUARDS.ROOM_MEMBER_SILENT` over `ROOM_MEMBER` only for spam surfaces — it refuses without emitting or evicting.
...
| Task | File |
|---|---|
| Add/modify a socket event | `hooks/useGameEvents.ts` (handler table) or `hooks/useGameActions.ts` (emit); server side `server/routes/index.js` (the row) plus the route module beside it |
| Socket route pipeline (rate limit, validate, guard) | `server/routes/register.js`; guards in `server/routes/guards.js` |
...
| Add a friend from a game | `server/controllers/roomFriendController.js` — the client names a co-player by SOCKET id and the server resolves the account, after checking BOTH sockets are in the room; account ids NEVER go on the wire. Guests, blocks and departed sockets are filtered server-side. Client `components/game/AddFriendsFromGame.tsx`, inside `GameSummary` |
| Friend presence + invites | `server/utils/presence.js` — presence is DERIVED from the live socket map, never stored, and the scan must never become a `user:<id>` room (room codes share that namespace); `server/controllers/friendInviteController.js` (friendship, sender-in-room, capacity, per-pair cooldown, all refusals silent); client `state/friendsSlice.ts`, `components/FriendInviteToast.tsx`, `components/game/InviteFriendDialog.tsx` (mounted ONCE by Grid — RoomPanel renders in both layout clusters) |
| Friends (mutual graph, codes, blocks) | `server/domain/friendCode.js` (PURE — alphabet, generation AND the validation rule, which cannot live in `validation.js`: that sits below `domain/` in the layer order); `server/data/friendsRepo.js` (caps, reciprocal-accept, block semantics); `server/controllers/friendsController.js` for the REST half, `server/routes/friends.js` for the three ROOM-scoped socket routes; client `lib/friendsApi.ts` + `app/profile/FriendsPanel.tsx`. A block placed ON you answers exactly like an unknown code — never name it. Plan: `SOCIAL_PRD.md` |
| Board pings ("look here") | `pingCell`/`playerPing`; handler in `server/routes/social.js` beside hover and emote, suppressed in PVP like hover, and sharing the emote's expression bucket from its row in `server/routes/index.js`; click interception is ONE capture listener on the grid in `components/game/Board.tsx` reading each cell's `data-row`/`data-col`, never per-branch inside `Cell.tsx`; ring in `components/game/PingLayer.tsx`; arming in `state/inputSlice.ts`; `P` in `hooks/useKeyboardControls.ts` |
| Reactions (emotes) | catalog in `shared/emotes.js` (ids + labels, imported by both halves); art in `components/ds/emoteArt.ts` (PURE, token-painted) drawn by `Emote.tsx`; tray + feed in `components/game/EmoteBar.tsx`; feed state in `state/roomSlice.ts`; lifetime and wording in `lib/emotes.ts`; server handler in `server/routes/social.js` behind `isValidEmoteId` and the shared expression bucket in `server/domain/rateLimit.js`. Plan: `SOCIAL_PRD.md` |
...
| Seasonal (holiday) palettes — windows, precedence, the override | `lib/holidays.ts`; the `data-theme` blocks live with every other palette in `app/tokens.css` |
| Mine/flag sprites | art in `components/ds/spriteArt.ts` (PURE — importable by `lib/`), drawn by `sprites.tsx` (the `<symbol>` defs and the `<Sprite>` that points at them); grids are 16x16 character rows like `icons.tsx` (shared renderer in `pixelArt.ts`). GENERAL sets are pinnable via `settings.spriteSet` (picker: `components/SpriteSetCards.tsx`, wiring: `components/SpriteDefsHost.tsx`); SEASONAL pairs are never pinnable and win over the pin while their window paints. Every set is on `/ds` |
...
1. **The server's layers are enforced, not suggested.** `tests/layering.test.js` derives the import graph from source and fails on a cycle, on any module importing a higher layer, or on anything in `domain/` reaching outside it. Order, lowest first: `config`/`validation` → the io and Redis singletons → `domain/` (pure) → `data/` → `utils/` → `game/` → `controllers/` → `routes/` → `server.js`. This exists because `gameUtils` and `playerUtils` used to require each other, which made `resetGame()` throw *depending on which file node loaded first*; `tests/resetGame.test.js` still guards that one function and its require order is load-bearing. **`shared/` is the one exception, at every layer including `domain/`** — it holds no I/O and belongs to both deployables, so `importsOf` filters it out of the graph entirely. That means the test cannot catch anything about it: a `shared/` module that grew a Redis call would sail straight through. Keep `shared/` pure.
2. **Adding a socket event touches four places**: `shared/events.js` (the name), `shared/socketPayloads.ts` (the payload), the server side (a row in `server/routes/index.js` for inbound, the emit for outbound), and the client table in `hooks/useGameEvents.ts`. `server/tests/events.test.js` fails if they drift. It was five until `shared/events.js` started freezing its objects — `Object.freeze` makes TypeScript infer `'boardUpdate'` instead of widening to `string`, which is what a hand-written `events.d.ts` used to buy. **Don't unfreeze them**: nothing breaks loudly, the handler table just degrades to `any`.
...
10. **Board records are keyed by dimensions, mine count and HOW MANY CLEARED IT — never by the size/difficulty labels.** `setDimensions` gives a joining player the room's numbers and leaves `boardSize`/`difficulty` at whatever they last picked, so anything keyed on a label files a joiner's data under a board they never played; `labelForKey` derives the display name back from the numbers. The group count is part of the identity because two people split a board and finish faster than one person can by construction — with one slot per board the group time takes it and no solo run is ever a record again, which is the bug the server's rows had until the `group-board-best-keys` migration. **The rule lives in `shared/boardKeys.js` and both halves import it**: the client to look a record up, the server to write one, and a PVP race counts as ONE player on both sides (`playersForClear`) because you clear the whole board yourself. Server-side the key is built in exactly one place — `utils/statsRecorder`, from the board and the room a game-over site already reports — and `statsRepo` reads the count back out of the key (`playersFromKey`) rather than deriving it again; a second derivation beside the key is how a row ends up keyed one way and captioned another. Signed in, the ACCOUNT's records are what the game shows (`hooks/useBestTime.ts`); localStorage is the guest copy, the fallback when they cannot be fetched, and folded into the account once on first sign-in (`BestsSync`, watermark in `lib/bestTimes.ts`). The daily files no board record at all — a different board every day would otherwise become the record for a board nobody can replay.
11. **A seasonal palette is PAINT, never DATA.** A holiday overrides `settings.theme` for the length of its window and must not write to it — someone who set Game Boy in March expects it back in November. That is why the override is resolved rather than folded into the blob (`applySideEffects` in `state/settingsSlice.ts`, and again inside `NO_FLASH_SCRIPT`), and why picking the already-lit holiday card is a deliberate no-op. Switching to anything else records `seasonalDismissed` for that occurrence only. Seasonal ids stay in `VALID_THEME_IDS` and on `/ds` all year — dropping them out of season would invalidate a stored value and hide the palette from the contrast audit. **The resolved value lives in the store as `seasonalOverride`, not in a component**: it moves on the CLOCK with nothing else changing, so anything deriving it from `new Date()` during render goes stale at midnight with no state change to re-render it. `refreshSeasonal` advances it — armed one timer at a time from `msUntilLocalMidnight`, since whole-day windows can only turn over at local midnight. **The mine and flag sprites follow the same rule.** They are pixel art (`components/ds/sprites.tsx`), mounted ONCE as two `<symbol>`s by `<SpriteDefs>` in the layout; every cell holds a two-node `<use>` pointing at them. That indirection is what keeps a holiday from costing a render: the defs follow the `data-theme` attribute — through a MutationObserver, **not** the store, because the no-flash script, the settings slice and `/ds` all set it and only the attribute knows all three — and `<use>` is a live reference, so every flag on the board redraws without React touching a cell. Inlining the art per cell instead would put ~40 rects in each of up to 99 mine cells. The default pair and every GENERAL set are drawn in TOKENS rather than literals, because they paint on twelve palettes: a black bomb vanishes on Game Boy, whose mine cell is its darkest green; only the seasonal pairs may use literal hex, each appearing on its own palette alone. The picker offers GENERAL sets only (`settings.spriteSet`), and the pin obeys the same paint-over-data rule as the theme: while a holiday window is open its pair wins over the pin, and the pin comes back when the window closes. Seasonal ids are not valid pins — `sanitizeSettings` drops them.
12. **Mine density has a measured ceiling.** Difficulty is a density in `shared/boardConfig.js`, and `Extreme` sits at 20.6% because that is the highest the no-guess generator can actually deliver — above it the retry loop exhausts and falls back to a guessy board *silently*. Don't raise a density, or lower `DEFAULT_MAX_ATTEMPTS`, without re-measuring the solvable rate; see ARCHITECTURE.md §5. Board dimensions come from the size axis, mines are always derived — never add a hand-typed mine count back.
