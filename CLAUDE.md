...
11. **A seasonal palette is PAINT, never DATA.** A holiday overrides `settings.theme` for the length of its window and must not write to it — someone who set Game Boy in March expects it back in November. That is why the override is resolved at every paint site (`applySideEffects` in `state/settingsSlice.ts`, and again inside `NO_FLASH_SCRIPT`) rather than folded into the blob, and why picking the already-lit holiday card is a deliberate no-op. Switching to anything else records `seasonalDismissed` for that occurrence only. Seasonal ids stay in `VALID_THEME_IDS` and on `/ds` all year — dropping them out of season would invalidate a stored value and hide the palette from the contrast audit.
