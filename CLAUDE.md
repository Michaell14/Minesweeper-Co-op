...
| Mine/flag sprites, per palette | art in `components/ds/spriteArt.ts` (PURE — importable by `lib/`), drawn by `sprites.tsx` (the `<symbol>` defs and the `<Sprite>` that points at them); grids are 16x16 character rows like `icons.tsx` (shared renderer in `pixelArt.ts`). Not user-selectable — the art follows the palette, so seasonal pairs appear only during their holiday windows. Every set is on `/ds` |
