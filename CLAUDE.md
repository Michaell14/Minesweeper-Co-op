...
| Mine/flag sprites, per palette | `components/ds/sprites.tsx` — the art, the `<symbol>` defs and the `<Sprite>` that points at them; grids are 16x16 character rows like `icons.tsx` (shared renderer in `pixelArt.ts`). Every set is on `/ds` |
