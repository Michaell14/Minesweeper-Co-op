 ...
 * setting, or a group that stops reflecting what the blob holds.
 */
...
describe('the sprite-set picker', () => {
    it('offers every pinnable set plus following the palette', () => {
        ...
        expect(screen.getByRole('radio', { name: /Halloween/ })).toBeDefined();
    });
    ...
    it('picking a set writes the pin to settings', () => {
        ...
        expect(useMinesweeperStore.getState().settings.spriteSet).toBe('halloween');
    });

    it('picking Match palette clears the pin back to null', () => {
        useMinesweeperStore.setState((state) => ({
            settings: { ...state.settings, spriteSet: 'christmas' },
        }));
        render(<SpriteSetCards name="test-sprites" />);

        expect(
            (screen.getByRole('radio', { name: /Christmas/ }) as HTMLInputElement).checked,
        ).toBe(true);
