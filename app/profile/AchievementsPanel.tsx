...
/**
 * The achievement shelf: pure presentation over the profile payload. Locked
 * entries render too, with progress from the same `metricsFrom` the server
 * awards on, so bar and badge cannot disagree. A HIDDEN entry shows neither
 * name nor description until earned but still occupies a tile. The first few
 * tiles are always on show; the rest sit behind a native `<details>`, which
 * brings keyboard operation, the disclosure role and the expanded state itself.
 */
...
export default function AchievementsPanel({
    ...
    /*
     * Opens itself when there is something new BEHIND THE FOLD. Scoped to
     * `rest`: a badge on a preview tile is already on screen.
     */
    ...
    const tile = (achievement: (typeof ACHIEVEMENTS)[number]) => {
        ...
        // guard stays here: the metrics keep meeting the threshold after the badge lands.
    ...
    return (
        <section aria-labelledby="profile-achievements" className="mb-8">
            <Panel title={<span id="profile-achievements">Achievements</span>}>
                ...
                <ul
                    className={`${GRID} mt-4`}
                    /* Explicit despite the tag: WebKit strips list semantics from list-style:none lists. */
                    role="list"
                ...
                {rest.length > 0 && (
                    <details className="group mt-2" open={hasHiddenNews}>
                        <summary
                            /*
                             * `list-none` kills the marker in Chrome/Firefox;
                             * Safari needs the -webkit pseudo-element too. `py-2`
                             * is the 24px tap target. The row layout hangs off an
                             * inner span so this stays `display: list-item`: a
                             * flex summary loses its disclosure triangle in Safari.
                             */
